...
{emp e;
   ...
   connect(ui->playm, &QPushButton::clicked, reproductor, &QMediaPlayer::play);
   connect(ui->pause, &QPushButton::clicked, reproductor, &QMediaPlayer::pause);
...
void gestion_zouhour::on_pushButton_4_clicked()
{
...
void gestion_zouhour::on_verticalSlider_sliderMoved(int position)
{reproductor->setVolume(position);
}

void gestion_zouhour::on_verticalSlider_actionTriggered(int action)
{
   reproductor->setVolume(action);
}
